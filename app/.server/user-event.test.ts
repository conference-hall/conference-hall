...
describe('UserEvent', () => {
  ...
  describe('#delete', () => {
    ...
    it('deletes an event and its relations', async () => {
      ...
      await UserEvent.for(owner.id, team.slug, event.slug).delete();
    ...
    it('throws an error if user is not owner', async () => {
      await expect(UserEvent.for(member.id, team.slug, event.slug).delete()).rejects.toThrowError(
        ForbiddenOperationError,
      );
    });
    ...
    it('throws an error if user does not belong to event team', async () => {
      const user = await userFactory();
      await expect(UserEvent.for(user.id, team.slug, event.slug).delete()).rejects.toThrowError(
        ForbiddenOperationError,
      );
    });
  });
