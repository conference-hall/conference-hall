...
export async function sendSubmittedTalkSlackMessage(eventId: string, proposalId: string) {
  ...
  const proposal = await db.proposal.findUnique({
    where: { id: proposalId },
    include: { newSpeakers: true, formats: true, categories: true },
  });
