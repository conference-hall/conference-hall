...
describe('Job: exportToOpenPlanner', () => {
  ...
  it('exports sessions and speakers to OpenPlanner', async () => {
    ...
    expect(postSessionsAndSpeakersMock).toHaveBeenCalledWith('open-planner-event-id', 'open-planner-api-key', {
      sessions: [
        {
          ...
          language: 'en',
          speakerIds: proposal2.newSpeakers.map((s) => s.id),
        ...
        },
        {
          ...
          language: 'en',
          speakerIds: proposal1.newSpeakers.map((s) => s.id),
          formatId: format.id,
      ...
      speakers: [
        {
          id: proposal1.newSpeakers.at(0)?.id,
          name: proposal1.newSpeakers.at(0)?.name,
          bio: proposal1.newSpeakers.at(0)?.bio,
          company: proposal1.newSpeakers.at(0)?.company,
          photoUrl: proposal1.newSpeakers.at(0)?.picture,
          socials: [],
        },
      ],
    });
