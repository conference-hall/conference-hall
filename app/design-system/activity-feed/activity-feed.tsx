...
function Entry({ marker, children, withLine, className }: EntryProps) {
  return (
    <li>
      <article className="relative flex gap-x-4">
        <div className="flex flex-col items-center w-8 shrink-0">
          {marker}
          {withLine ? <Line /> : null}
        </div>
        <div className={cx('flex-1 pb-4 md:pb-8', className)}>{children}</div>
      </article>
    </li>
  );
}
