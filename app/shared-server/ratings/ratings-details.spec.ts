...
describe('#RatingsDetails', () => {
  it('computes ratings info from user ratings', () => {
    ...
    expect(ratings.ofUser('uid1')).toEqual({ rating: rating1.rating, feeling: rating1.feeling });
    expect(ratings.ofMembers()).toEqual([
      { id: 'uid3', name: 'Bob doe', picture: 'b.png', rating: 0, feeling: 'NEGATIVE' },
      { id: 'uid2', name: 'Jane doe', picture: 'd.png', rating: null, feeling: 'NO_OPINION' },
      { id: 'uid1', name: 'John doe', picture: 'j.png', rating: 5, feeling: 'POSITIVE' },
    ]);
  });
  ...
  it('computes default values if no ratings', () => {
    ...
    expect(ratings.ofUser('uid1')).toEqual({ rating: null, feeling: null });
    expect(ratings.ofMembers()).toEqual([]);
  });
});
