...
describe('#addCoSpeakerToProposal', () => {
  ...
  it('adds the speaker to the proposal and the talk', async () => {
    ...
    await addCoSpeakerToProposal({ invitationId: invite?.id, coSpeakerId: cospeaker.id });
  ...
  it('throws an error when invitation not found', async () => {
    const speaker = await userFactory();
    const result = await addCoSpeakerToProposal({ invitationId: 'XXX', coSpeakerId: speaker.id });
    expect(result.errors[0].message).toBe('Invitation not found');
  });
});
