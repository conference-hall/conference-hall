...
export function SearchPagination({ pathname, current, total, className }: Props) {
  ...
  return (
    <nav className={c('border-t border-gray-200 px-4 flex items-center justify-center sm:px-0', className)}>
      {pages.map((page) => {
        ...
        if (showPageButton(page, current, total)) {
          ...
          return <Text className="text-gray-500 pt-4 px-4 inline-flex items-center text-sm font-medium">...</Text>;
        }
        return null;
      })}
    </nav>
