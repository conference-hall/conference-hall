...
type Props = { email: string; hasPassword: boolean };

export function EmailPasswordSection({ email, hasPassword }: Props) {
          ...
          <List.Content aria-label={t('settings.account.authentication-methods.list')}>
            ...
            <List.Row className="min-h-16 flex-col justify-between gap-4 p-4 sm:flex-row">
              ...
              {hasPassword ? (
                ...
                <AddPasswordModal email={email} open={openPassword} onClose={() => setOpenPassword(false)} />
              )}
            </List.Row>
          </List.Content>
        </List>
...
function ChangeEmailModal({ email, open, onClose }: ChangeEmailModalProps) {
  ...
  const onChangeEmail = async (event: FormEvent) => {
    ...
    if (!error) {
      toast.success(`Lien envoyé à ${newEmail}`);
      onClose();
    } else {
  ...
  return (
    <Modal title="Modifier votre email" onClose={onClose} open={open}>
      <Modal.Content className="space-y-4">
        <div className="flex gap-1">
          <Text variant="secondary">Email actuel : </Text>
          <Text>{email}</Text>
        </div>
        ...
        <Callout>
          Vous recevrez un lien de confirmation à cette adresse. Votre email actuel restera actif jusqu'à la
          confirmation.
        </Callout>
      </Modal.Content>
      <Modal.Actions>
...
type AddPasswordModalProps = { email: string; open: boolean; onClose: VoidFunction };

function AddPasswordModal({ email, open, onClose }: AddPasswordModalProps) {
  const { t } = useTranslation();
  ...
  return (
    <Modal title="Définir votre mot de passe" onClose={onClose} open={open}>
      <Modal.Content className="space-y-4">
        <Text>
          Vous allez recevoir un lien à <strong>{email}</strong> pour créer votre mot de passe.
        </Text>
      </Modal.Content>
      <Modal.Actions>
        <Button type="button" variant="secondary" onClick={onClose}>
        ...
        <Button type="button" variant="primary" onClick={onAddPassword}>
          Envoyer le lien
        </Button>
      </Modal.Actions>
    </Modal>
...
function ChangePasswordModal({ open, onClose }: ChangePasswordModalProps) {
  ...
  return (
    <Modal title="Modifier votre mot de passe" onClose={onClose} open={open}>
      <Modal.Content className="space-y-4">
