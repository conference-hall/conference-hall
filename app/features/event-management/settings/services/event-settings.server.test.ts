...
describe('EventSettings', () => {
  describe('#get', () => {
    it('returns the event for organizer', async () => {
      ...
      const result = await EventSettings.for(user.id, team.slug, event.slug).get();
    ...
    it('throws an error if user does not belong to event team', async () => {
      ...
      await expect(EventSettings.for(user.id, team.slug, event.slug).get()).rejects.toThrowError(EventNotFoundError);
    });
