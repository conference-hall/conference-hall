...
export class ProposalSearchBuilder {
  ...
  async proposalsByPage(pagination: Pagination) {
    ...
    const speakerIds = this.options.withSpeakers ? ids : [];
    const [speakersMap, tagsMap] = await Promise.all([this.fetchSpeakers(speakerIds), this.fetchTags(ids)]);
  ...
  private buildOrderByClause(): Prisma.Sql {
    switch (this.filters.sort) {
      case 'highest':
        return Prisma.sql`COALESCE(review_agg.avg_rating, 0) DESC, p.title ASC`;
      case 'lowest':
        return Prisma.sql`COALESCE(review_agg.avg_rating, 0) ASC, p.title ASC`;
      case 'oldest':
        return Prisma.sql`p."submittedAt" ASC, p.title ASC`;
      case 'most-comments':
        return Prisma.sql`COALESCE(comment_count.count, 0) DESC, p.title ASC`;
      case 'fewest-comments':
        return Prisma.sql`COALESCE(comment_count.count, 0) ASC, p.title ASC`;
      default:
