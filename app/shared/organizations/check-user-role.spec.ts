...
describe('#checkUserRole', () => {
  ...
  it('returns the organizer role when user has access to the event', async () => {
    ...
    const result = await checkUserRole(organization.slug, event.slug, user.id);
  ...
  it('returns the organizer role if user role part of accepted ones', async () => {
    ...
    const result = await checkUserRole(organization.slug, event.slug, user.id, ['OWNER']);
  ...
  it('throws an error if user role is not in the accepted role list', async () => {
    ...
    const organization = await organizationFactory({ owners: [user] });
    const event = await eventFactory({ organization });
    await expect(checkUserRole(organization.slug, event.slug, user.id, ['MEMBER'])).rejects.toThrowError(
  ...
  it('throws an error if user role is not part of the organization', async () => {
    ...
    const event = await eventFactory({ organization });
    await expect(checkUserRole(organization.slug, event.slug, user.id)).rejects.toThrowError(ForbiddenOperationError);
  });

  it('throws an error if event does not exist', async () => {
    const user = await userFactory();
    const organization = await organizationFactory({ owners: [user] });
    await expect(checkUserRole(organization.slug, 'AAAA', user.id)).rejects.toThrowError(ForbiddenOperationError);
  });
});
