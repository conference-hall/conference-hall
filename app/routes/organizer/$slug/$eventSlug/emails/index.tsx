...
import { sendAllAcceptationCampaign } from '~/services/organizers/emails-campaign.server';
...
export const action = async ({ request, params }: ActionArgs) => {
  const { uid } = await sessionRequired(request);
  const { slug, eventSlug } = params;
  await sendAllAcceptationCampaign(slug!, eventSlug!, uid);
  return json(null);
};
  ...
  return (
    <>
      ...
      <dl className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        <StatIndicator label="Accepted proposals">{results.length}</StatIndicator>
