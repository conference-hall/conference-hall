...
export const seed = async () => {
  ...
  const format1 = await eventFormatFactory({ event, attributes: { id: 'f1', name: 'Format 1' } });
  const format2 = await eventFormatFactory({ event, attributes: { id: 'f2', name: 'Format 2' } });
  const category1 = await eventCategoryFactory({ event, attributes: { id: 'c1', name: 'Category 1' } });
  const category2 = await eventCategoryFactory({ event, attributes: { id: 'c2', name: 'Category 2' } });
  ...
  await eventFactory({
    team,
    traits: ['conference-cfp-past', 'private'],
    attributes: { name: 'Conference 2', slug: 'conference-2', reviewEnabled: false },
  });
};
