...
test.describe('as a team owner', () => {
  test('can manage the team but cannot leave the team', async ({ context, page }) => {
  ...
  test('filter members', async ({ context, page }) => {
    const owner = await userLoggedFactory(context, { traits: ['clark-kent'], withPasswordAccount: true });
  ...
  test('search members', async ({ context, page }) => {
    const owner = await userLoggedFactory(context, { traits: ['clark-kent'], withPasswordAccount: true });
    const member = await userFactory({ traits: ['bruce-wayne'] });
    const team = await teamFactory({ owners: [owner], members: [member] });
  ...
  test('can manage team members', async ({ context, page }) => {
    const owner = await userLoggedFactory(context, { traits: ['clark-kent'], withPasswordAccount: true });
    const member = await userFactory({ traits: ['bruce-wayne'] });
    const team = await teamFactory({ owners: [owner], members: [member] });
...
test.describe('as a team member', () => {
  test('cannot edit the team but can leave the team', async ({ context, page }) => {
