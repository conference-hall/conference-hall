import { DEFAULT_PASSWORD } from 'tests/factories/users.ts';
...
test('display settings page', async ({ page, context }) => {
...
test('set a password', async ({ page, context }) => {
  await userLoggedFactory(context, {
    attributes: { email: 'original@example.net' },
...
test('delete account', async ({ page, context }) => {
