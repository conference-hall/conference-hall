...
test.describe('Account information', () => {
  ...
  test('Update account information', async ({ page }) => {
    await userFactory({ traits: ['clark-kent'] });

    const securityPage = new SettingsSecurityPage(page);
    await securityPage.goto();

    await securityPage.fillAccountInfo('John Doe', 'john.doe@email.com', 'https://john-doe.com/photo.jpg');
    await expect(securityPage.toast).toHaveText('Account information updated.');

    await securityPage.goto();
    await securityPage.fillAccountInfo('', '', '');
    await expect(await securityPage.fullNameError()).toHaveText('Required');
    await expect(await securityPage.emailError()).toHaveText('Required');
  });
});
