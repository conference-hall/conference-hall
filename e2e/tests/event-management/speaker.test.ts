...
test.describe('Speaker Page', () => {
  ...
  test.beforeEach(async () => {
    const owner = await userFactory({ withAuthSession: true });
  ...
  test('displays proposals with status badges', async ({ page }) => {
    ...
    await expect(page).toHaveURL(
      `/team/${team.slug}/${event.slug}/proposals/${proposal.routeId}?speakers=${eventSpeaker.id}`,
    );
  });
...
test.describe('New Proposal Button Permissions', () => {
  test.describe('As a reviewer', () => {
    ...
    test.beforeEach(async () => {
      const reviewer = await userFactory({ withAuthSession: true });
